...
export class TaskConfigurationComponent implements OnInit {
  ...
  public filestackImageUrls: string[] = [];
  ...
  ngOnInit(): void {
    ...
    this.whatsFormArray.valueChanges.subscribe((whats: TaskWhatData[]) => {
      ...
      //if (whats.length > 0) {
      //  let lastWhat = whats[whats.length - 1];
      //  // SELECT
      //  lastWhat.systemDefinedActionTypeKey
      //}
    });
  ...
  onWhatSelected(what: ExtendedTaskActionData) {
    if (what.defaultAssingedUserId) {
      let userWho: TaskWhoData = this.allWhos.find(w => w.referenceId === what.defaultAssingedUserId && w.typeKey === "USER");
      //(this.taskConfigurationForm.controls.whos as FormArray).clear();
      //(this.taskConfigurationForm.controls.whos as FormArray).push([userWho]);
      this.taskConfigurationForm.controls.whos.setValue([userWho]);

    }
    else if (what.defaultAssingedUserSubGroupId) {
      let subGroupWho: TaskWhoData = this.allWhos.find(w => w.referenceId === what.defaultAssingedUserSubGroupId && w.typeKey === "SUBGROUP");
      this.taskConfigurationForm.controls.whos.setValue([subGroupWho]);
    }
    else if (what.defaultAssingedUserGroupId) {
      let groupWho: TaskWhoData = this.allWhos.find(w => w.referenceId === what.defaultAssingedUserGroupId && w.typeKey === "GROUP");
      this.taskConfigurationForm.controls.whos.setValue([groupWho]);
    }
  }
  ...
  private _generateTaskRecurringForm(taskConfiguration: TaskConfigurationDetailsData): FormGroup {
    ...
    return this._formBuilder.group({
      ...
      taskRecurringEveryForm: this._generateTaskRecurringEveryForm(taskConfiguration.recurringEveryTaskOptions),
    });
  }
  ...
  private _generateTaskRecurringEveryForm(options: RecurringEveryTaskOptions): FormGroup {
    //if (!options) {
    //  options = this._createNewSpecificTimesRecurringTaskOptions();
    //}
    ...
    return this._formBuilder.group({
      ...
      startsAtDate: [startsAtMoment, Validators.required],
      startsAtTime: [DateHelper.getMomentTime(startsAtMoment), Validators.required],
      everyNumberOfDays: [options.everyNumberOfDays, Validators.required]
    });
  }
  ...
  private _createInsertTaskConfigurationRequest(): InsertTaskConfigurationCommand {
    ...
    let insertRequest = new InsertTaskConfigurationCommand({
      ...
      filestackImageUrls: this.filestackImageUrls,
      ...
      recurringEveryTaskOptions: new SaveRecurringEveryTaskOptions({
        startsAtString: MomentDateHelper.getIsoDateTimeWithTime(formValues.taskRecurringForm.taskRecurringEveryForm.startsAtDate, formValues.taskRecurringForm.taskRecurringEveryForm.startsAtTime),
        repeatsForKey: formValues.taskRecurringForm.taskRecurringEveryForm.repeatsForKey,
        repeatsForNrDays: +formValues.taskRecurringForm.taskRecurringEveryForm.repeatsForNrDays,
        repeatsForNrOccurences: +formValues.taskRecurringForm.taskRecurringEveryForm.repeatsForNrOccurences,
        repeatsUntilTimeString: MomentDateHelper.getIsoDateTimeWithTime(formValues.taskRecurringForm.taskRecurringEveryForm.repeatsUntilDate, formValues.taskRecurringForm.taskRecurringEveryForm.repeatsUntilTime),
        everyNumberOfDays: +formValues.taskRecurringForm.taskRecurringEveryForm.everyNumberOfDays,
      }),
    });
  ...
  public onFilesChanged(filestackImageUrls: string[]) {
    console.log("ON FILES CHANGED: ", filestackImageUrls);
    this.filestackImageUrls = filestackImageUrls;
  }
}
