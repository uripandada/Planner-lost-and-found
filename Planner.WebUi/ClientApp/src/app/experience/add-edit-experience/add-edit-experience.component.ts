...
export class AddEditExperienceComponent implements OnInit {
  ...
  reservationList: BehaviorSubject<ReservationViewModel[]> = new BehaviorSubject<ReservationViewModel[]>(null);
  ...
  setReservationValue(name: string) {
    ...
    this.experienceForm.controls.checkOut.setValue(item.checkOut?.format('yyyy-MM-DD'));
    this.experienceForm.controls.reservationId.setValue(item.id);
    this.experienceForm.controls.vip.setValue(item.vip);
    this.experienceForm.controls.group.setValue(item.group);
  }
